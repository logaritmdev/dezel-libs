...
void StylerParser::parseStylesheet(StylerStylesheet &stylesheet) {
	Parser::parseStylesheet(stylesheet);
}

void StylerParser::parseStylesheet(StylerRuleset &ruleset) {
	while (parseRulesetStatement(ruleset));
}

void StylerParser::skipWhitespace() {
	while (
...
bool StylerParser::parseStatement(Stylesheet &ss) {

	Token token;
	TokenList tokens;

	StylerStylesheet *stylesheet = reinterpret_cast<StylerStylesheet *>(&ss);
...
bool StylerParser::parseAtRule(StylerStylesheet *stylesheet, StylerRuleset *ruleset) {
...
bool StylerParser::parseAtRule(StylerStylesheet &stylesheet) {
	return parseAtRule(&stylesheet, NULL);
}

bool StylerParser::parseAtRule(StylerRuleset &ruleset) {
	return parseAtRule(NULL, &ruleset);
}

bool StylerParser::parseAtRuleValue(TokenList &rule) {
...
bool StylerParser::parseVariable(TokenList &value) {
...
bool StylerParser::parseSelector(TokenList &tokens) {
...
bool StylerParser::parseSelectorVariable(TokenList &selector) {
	return false;
}

bool StylerParser::parseRuleset(TokenList &tokens, StylerStylesheet *stylesheet, StylerRuleset *parentRuleset) {

	StylerRuleset* ruleset;
	...
	selector = new StylerSelector();
	stylerSelectorParser.parse(tokens, *selector);

	// Create the ruleset and parse ruleset statements.
	if (parentRuleset == NULL) {
		ruleset = stylesheet->createStylerRuleset(*selector);
	} else {
...
bool StylerParser::parseRuleset(StylerStylesheet &parent, TokenList &selector) {
	return parseRuleset(selector, &parent, NULL);
}

bool StylerParser::parseRuleset(StylerRuleset &parent, TokenList &selector) {
	return parseRuleset(selector, NULL, &parent);
}

void StylerParser::parseMediaQueryRuleset(Token &mediatoken, StylerRuleset &parent) {

	StylerMediaQueryRuleset *query;
	TokenList selector;
...
bool StylerParser::parsePropertyVariable(TokenList &selector) {
...
bool StylerParser::parseRulesetStatement(StylerRuleset &ruleset) {
...
bool StylerParser::parseComment(StylerRuleset &ruleset) {
...
bool StylerParser::parseExtension(TokenList &statement, StylerRuleset &ruleset) {
...
bool StylerParser::parseDeclaration(TokenList &tokens,
	size_t property_i,
	StylerRuleset &ruleset) {
	StylerDeclaration *d;
...
bool StylerParser::parseMixin(TokenList &tokens, StylerRuleset &parent) {
	return parseMixin(tokens, &parent, NULL);
}

bool StylerParser::parseMixin(TokenList &tokens, StylerStylesheet &parent) {
	return parseMixin(tokens, NULL, &parent);
}

bool StylerParser::parseMixin(TokenList &tokens, StylerRuleset *parent_r, StylerStylesheet *parent_s) {
...
void StylerParser::parseMixinArguments(TokenList::const_iterator &i,
...
bool StylerParser::parseImportStatement(TokenList &statement,
	StylerStylesheet *stylesheet,
	StylerRuleset *ruleset) {
...
unsigned int StylerParser::parseImportDirective(Token &t) {
...
bool StylerParser::importFile(Token uri,
	StylerStylesheet *stylesheet,
	StylerRuleset *ruleset,
	...
	StylerTokenizer tokenizer(in, relative_filename_cpy);
	StylerParser parser(tokenizer, sources, (directive & IMPORT_REFERENCE));
...
bool StylerParser::findFile(Token &uri, std::string &filename) {
	ifstream *in;
	size_t pos;
	std::string source;
	std::list<const char *>::iterator i;
...
void StylerParser::parseLessMediaQuery(Token &mediatoken, StylerStylesheet &stylesheet) {

	StylerMediaQuery *query;
	TokenList selector;
