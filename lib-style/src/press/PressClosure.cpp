#include "PressClosure.h"
#include "ProcessingContext.h"
#include "PressRuleset.h"

namespace Press {

PressClosure::PressClosure(const PressRuleset &ruleset, const PressMixinCall &stack) {
	this->ruleset = &ruleset;
	this->stack = &stack;
}

bool PressClosure::call(PressMixinArguments &args,
	Ruleset &target,
	ProcessingContext &context,
	bool defaultVal) const {
	return ruleset->call(args, target, context, defaultVal);
}

bool PressClosure::call(PressMixinArguments &args,
	Stylesheet &s,
	ProcessingContext &context,
	bool defaultVal) const {
	return ruleset->call(args, s, context, defaultVal);
}

void PressClosure::getFunctions(std::list<const PressFunction *> &functionList,
	const PressMixin &mixin,
	TokenList::const_iterator offset,
	const ProcessingContext &context) const {
	const std::list<PressRuleset *> &nestedRules = ruleset->getNestedRules();
	const std::list<PressClosure *> *closures = context.getClosures(ruleset);
	std::list<PressRuleset *>::const_iterator r_it;
	std::list<PressClosure *>::const_iterator c_it;
	TokenList::const_iterator offset2;

	offset2 = getPressSelector().walk(offset, mixin.name.end());

	if (offset2 == offset)
		return;

	while (offset2 != mixin.name.end() &&
		   ((*offset2).type == Token::WHITESPACE || *offset2 == ">")) {
		offset2++;
	}

	if (getPressSelector().matchArguments(mixin.arguments)) {
		if (offset2 == mixin.name.end()) {
			functionList.push_back(this);
		} else {
			for (r_it = nestedRules.begin(); r_it != nestedRules.end(); r_it++) {
				(*r_it)->getFunctions(functionList, mixin, offset2, context);
			}
			if (closures != NULL) {
				for (c_it = closures->begin(); c_it != closures->end(); c_it++) {
					(*c_it)->getFunctions(functionList, mixin, offset2, context);
				}
			}
		}
	}
}

const PressSelector &PressClosure::getPressSelector() const {
	return ruleset->getPressSelector();
}

const TokenList *PressClosure::getVariable(const std::string &key,
	const ProcessingContext &context) const {
	const TokenList *t;

	if ((t = ruleset->getVariable(key, context)) != NULL)
		return t;
	return this->stack->getVariable(key, context);
}

void PressClosure::getLocalFunctions(std::list<const PressFunction *> &functionList,
	const PressMixin &mixin,
	const ProcessingContext &context) const {
	ruleset->getFunctions(functionList, mixin, mixin.name.begin(), context);
	if (functionList.empty())
		stack->getFunctions(functionList, mixin, context);
}

}
